...
public class NacosServiceDiscoveryService : INacosServiceDiscoveryService
{
    ...
    public async Task<List<string>> GetIdentityServiceUrlsAsync()
    {
        try
        {
            _logger.LogInformation("开始从 Nacos获取身份服务实例，服务名: {ServiceName}", IdentityServiceName);
            ...
            _logger.LogInformation("从 Nacos 获取到 {Count} 个实例", instances?.Count ?? 0);
            
            if (instances == null || !instances.Any())
            {
                _logger.LogWarning("从 Nacos 获取身份服务实例失败或没有可用实例");
            ...
            _logger.LogInformation("从 Nacos 获取到 {Count} 个身份服务实例", urls.Count);
            return urls;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "从 Nacos 获取身份服务实例时发生错误");
            return new List<string>();
        }
    }
