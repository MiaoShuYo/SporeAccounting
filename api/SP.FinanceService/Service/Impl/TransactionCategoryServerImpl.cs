...
public class TransactionCategoryServerImpl : ITransactionCategoryServer
{
    ...
    /// 新增收支分类
     /// 1.检查是否存在同名分类
     /// 2.如果指定了父级分类，检查父级分类是否存在以及父级分类与子分类的类型是否一致
     /// 3.创建新的收支分类实体并保存到数据库
    /// </summary>
    /// <param name="category"></param>
    /// <returns>返回新建分类的ID</returns>
    ...
    /// 根据类型查询分类
    /// </summary>
    /// <param name="transactionCategoryEnmu">分类类型枚举</param>
    /// <returns>返回分类列表</returns>
    public List<TransactionCategoryResponse> QueryByType(TransactionCategoryEnmu transactionCategoryEnmu)
    {
        var categories = _dbContext.TransactionCategories
            .Where(c => c.Type == transactionCategoryEnmu && c.CreateUserId == _userId && !c.IsDeleted).ToList();
        List<TransactionCategoryResponse> categoryResponses =
            _automapper.Map<List<TransactionCategoryResponse>>(categories);
        return categoryResponses;
    }
