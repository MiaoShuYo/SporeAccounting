...
public class PermissionsMiddleware
{
        ...
        IConfiguration configuration,ISysUrlServer sysUrlServer)
    {
        ...
        if (requestPath != null && (requestPath.Contains("/api/SysUser/Login") ||
        ...
        else
        {
            ...
            var key = Encoding.UTF8.GetBytes(configuration["JWT:IssuerSigningKey"] ?? string.Empty);
            try
            {
                ...
                // 查询url的id
                SysUrl sysUrl = sysUrlServer.QueryByUrl(pathUrlNotParam);

                bool isUse = roleId != null && sysRoleUrlServer.IsRoleUseUrl(roleId, sysUrl.Id);
                if (!isUse)
                {
                    httpContext.Response.StatusCode = StatusCodes.Status403Forbidden;
                    await httpContext.Response.WriteAsync("权限不足：用户无权访问此资源。");
